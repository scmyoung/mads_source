 ...
 */
- (FBRequest_Scm*)openUrl:(NSString *)url
  ...
  [_request release];
  _request = [[FBRequest_Scm getRequestWithParams:params
...
- (void)authorizeWithFBAppAuth:(BOOL)tryFBAppAuth
  ...
  if ([device respondsToSelector:@selector(isMultitaskingSupported)] && [device isMultitaskingSupported]) {
    if (tryFBAppAuth) {
      ...
      NSString *fbAppUrl = [FBRequest_Scm serializeURL:urlPrefix params:params];
    ...
    if (trySafariAuth && !didOpenOtherApp) {
      ...
      NSString *fbAppUrl = [FBRequest_Scm serializeURL:loginDialogURL params:params];
      didOpenOtherApp = [[UIApplication sharedApplication] openURL:[NSURL URLWithString:fbAppUrl]];
    }
  }
  ...
  if (!didOpenOtherApp) {
    [_loginDialog release];
    _loginDialog = [[FBLoginDialog_Scm alloc] initWithURL:loginDialogURL
                                          loginParams:params
                                             delegate:self];
    [_loginDialog show];
  }
...
- (void)dialog:(NSString *)action
  ...
  if (action == kLogin) {
    ...
    _fbDialog = [[FBLoginDialog_Scm alloc] initWithURL:dialogURL loginParams:params delegate:self];
  } else {
    ...
    _fbDialog = [[FBDialog_Scm alloc] initWithURL:dialogURL params:params delegate:delegate];
  }
